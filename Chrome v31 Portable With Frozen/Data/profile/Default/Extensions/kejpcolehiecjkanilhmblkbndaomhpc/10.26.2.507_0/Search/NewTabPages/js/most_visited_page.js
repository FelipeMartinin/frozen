...
cr.define('ntp', function() {
  ...
  var tileID = 0;
       ...
       ////////////////////////////////////////////////////////////////////////////////
  /**
   * MostVisited context menu. The class is designed to be used as a singleton with
   * the app that is currently showing a context menu stored in this.mostVisited_.
   * @constructor
   */
  function mostVisitedContextMenu() {
    this.__proto__ = mostVisitedContextMenu.prototype;
    this.initialize();
  }
  cr.addSingletonGetter(mostVisitedContextMenu);

  mostVisitedContextMenu.prototype = {
    initialize: function() {
      ...
      this.launchRegularTab_ = this.appendMenuItem_('mostvisited_launchtype_tab');
      this.launchRegularTab_.addEventListener('activate', this.onLaunchRegularTab_.bind(this));

      this.launchNewWindow_ = this.appendMenuItem_('mostvisited_launchtype_window');
      this.launchNewWindow_.addEventListener('activate', this.onLaunchNewWindow_.bind(this));

      this.launchIncognito_ = this.appendMenuItem_('mostvisited_launchtype_incognito');
      this.launchIncognito_.addEventListener('activate', this.onLaunchIncognito_.bind(this));
     ...
     */
    setupForMostVisited: function(mostVisited) {

      this.mostVisited_ = mostVisited;
...
//      this.uninstall_.disabled = !app.appData.mayDisable;
     ...
     * @private
     */
    onLaunchRegularTab_: function(e) {
       conduit.newtab.usage.CallUsage("MostVisited_RightClick", "ActionType", "NewTab");
       conduit.newtab.mostVisited.openSite(this.mostVisited_.href, conduit.newtab.mostVisited.OPEN_TYPE.open_new_tab);
    },
    onLaunchNewWindow_: function(e) {
      conduit.newtab.usage.CallUsage("MostVisited_RightClick", "ActionType", "window"); 
       conduit.newtab.mostVisited.openSite(this.mostVisited_.href, conduit.newtab.mostVisited.OPEN_TYPE.open_new_window);

    },
    onLaunchIncognito_: function(e) {
      conduit.newtab.usage.CallUsage("MostVisited_RightClick", "ActionType", "incognitoWindow"); 
       conduit.newtab.mostVisited.openSite(this.mostVisited_.href, conduit.newtab.mostVisited.OPEN_TYPE.open_in_incognito);

    },
    onUninstall_: function(e) {
       this.mostVisited_.removeFromChrome("right menu");
    }
  };
  ...
  MostVisited.prototype = {
    ...
    reset: function() {
      ...
      this.innerHTML =
          '<span class="thumbnail-wrapper fills-parent" id="most-visited-thumbnail-holder">' +
            ...
            '<img id="favicon-hidden" style="visibility:hidden;"/>' +
          '</span>' +
    ...
    updateForData: function(data) {
      ...
      // this.contextMenu.
       var MainDiv = this.querySelector('#most-visited-thumbnail-holder');
       var self = this;

      MainDiv.__defineGetter__('contextMenu', function() {
        return self.contextMenu;
      });
      MainDiv.addEventListener('contextmenu', cr.ui.contextMenuHandler);
       ////////////////////////////////////////////////////////////////////////////////
       

      var id = tileID++;
      var faviconImgId = 'most-visited-tile-' + id;
       this.id = faviconImgId;
      this.data_ = data;
      this.classList.add('focusable');

      var faviconDiv = this.querySelector('.favicon');
      var faviconImg = this.querySelector('#favicon-hidden');
      ...
      $(faviconImg)
          .load(function() 
             ...
             var dominant = getDominantColor(faviconImg);
             var dominantColor = "rgb(" + dominant[0] + ", " + dominant[1] + ", " + dominant[2] + ")";
             ntp.setStripeColor( faviconImgId, dominantColor );
             })
          .error(function() {

            var dominantColor = 'rgb(145,145,145)';
            ntp.setStripeColor( faviconImgId, dominantColor );
      ...
      this.querySelector('.thumbnail').style.backgroundImage = url(thumbnailUrl);
      this.querySelector('.thumbnail').style.backgroundPosition = "center";
      this.href = data.url;
    ...
    set stripeColor(color) {
      this.querySelector('.thumbnail-wrapper').style.borderColor = color;
    },
    ...
    handleClick_: function(e) {
      if (e.target.classList.contains('close-button')) {
        ...
        conduit.newtab.usage.CallUsage("MostVisited_click","tileIndex",this.index);
      }
    },
     ...
     */
    blacklist_: function(source) {
      this.showUndoNotification_();
      // KobyM
       var Tile = this;
         conduit.newtab.usage.CallUsage("MostVisited_remove", 'ActionType' , source);
      conduit.newtab.mostVisited.blackList(this.data_.url);
      Tile.reset();
    ...
    showUndoNotification_: function() {
      ...
      var self = this;
      var doUndo = function() {
         conduit.newtab.usage.CallUsage("MostVisited_undo_link");
      conduit.newtab.mostVisited.undoBlackList(data.url);
      ...
      var undoAll = {
        action: function() {
         conduit.newtab.usage.CallUsage("MostVisited_RestoreAll_link");
         conduit.newtab.mostVisited.clearBlackList();
     ...
     * singleton mostVisitedContextMenu. This function is called by the
     * ContextMenuHandler in response to the 'contextmenu' event.
     * @type {cr.ui.Menu}
     */
    get contextMenu() {
      var menu = mostVisitedContextMenu.getInstance();
      menu.setupForMostVisited(this);
      return menu.menu;
    }
  };
   ...
   * @extends {TilePage}
   */
  function MostVisitedPage(pageName) {
    var el = new TilePage(mostVisitedPageGridValues,pageName);
  ...
  MostVisitedPage.prototype = {
    ...
    initialize: function() {
      ...
      this.addEventListener('cardselected', this.handleCardSelected_);
    	cr.dispatchSimpleEvent(document, 'sectionready', true, true, loadTimeData.getInteger('most_visited_page_id'));
    },
    ...
    updateTiles_: function() {
	 ...
	 updateTile_: function(index, image, href) {
	 	for (var i = 0; i < this.data_.length; i++) {
	 		var page = this.data_[i];
	 		if (page.url == href) {
	 			page.image = image;
	 			for (i = 0; i < this.mostVisitedTiles_.length; i++) {
	 				var tile = this.mostVisitedTiles_[i];
	 				if (tile.href == page.url) {
	 					tile.updateForData(page);
	 					return;
	 				}
	 			}
