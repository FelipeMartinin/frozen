...
cr.define('ntp', function() {
  ...
  Tile.prototype = {
    ...
    initialize: function(contents) {
      this.footerTilesWrapper = document.getElementById('dot-list');
    ...
    onDragStart_: function(e) {
      ...
      // animation.
    	if (this.querySelector('.app') != null)
    		conduit.newtab.usage.CallUsage('Apps_Drag_and_drop');
      ...
      document.getElementById('card-slider-frame').appendChild(this.dragClone);
   ...
   * @extends {HTMLDivElement}
   */
  function TilePage(gridValues, pageName) {
    var el = cr.doc.createElement('div');
    el.gridValues_ = gridValues;
    el.pageName_ = pageName;
  ...
  TilePage.prototype = {
    ...
    initialize: function() {
      ...
      // Page title
      this.topMargin_.textContent = this.pageName_;
      ...
      // fix the "jump" when user is entering the page again - using the back button
      this.content_.onclick = function (e) {
          
          // Don't handle right or middle clicks
          if( (e.button == 1 || e.button == 2) || (( e.button == 0 ) && (e.metaKey || e.shiftKey || e.altKey || e.ctrlKey) )) {
              return;
          }
          if (e.srcElement.className == "thumbnail-shield fills-parent" || e.srcElement.className == "title") {
    ...
    set notification(node) {
      ...
      this.insertBefore(node, this.topMargin_.nextElementSibling);
    ...
    updateTopMargin_: function() {
      ...
      if (this.topMarginIsForWide_ != layout.wide) {
        ...
        this.topMargin_.style.marginBottom = this.animatedTopMarginPx_ +50+ 'px';
      }

      this.topMarginIsForWide_ = layout.wide;
      this.topMarginPx_ = newMargin;
       var margins = (this.topMarginPx_ - this.animatedTopMarginPx_);
      if (margins>0) {
          this.topMargin_.style.marginBottom = margins/2 +50+ 'px';
          this.topMargin_.style.marginTop = margins/2 +50+ 'px';

          this.footerTilesWrapper.style.width = (this.topMargin_.offsetWidth-500)+ 'px';
      }
    },
    ...
    positionNotification_: function() {
      if (this.notification && !this.notification.hidden) {
          this.notification.style.marginLeft = -(this.notification.offsetWidth/2)+"px";
    ...
    doUpdateScrollbars_: function() {
	  ...
	  //console.log('content.scrollHeight=' + content.scrollHeight + ', content.offsetTop=' + content.offsetTop + ', adjustedScrollHeight=' + adjustedScrollHeight);
