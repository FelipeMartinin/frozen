  ...
  CardSlider.prototype = {
    ...
    onMouseWheel_: function(e) {
      ...
      if (this.mouseWheelIsContinuous_) {
        ...
        if (Math.abs(this.mouseWheelScrollAmount_) >= 600) {
          ...
          this.selectCard(newCardIndex, 'mouse', true);
    ...
    addCardAtIndex: function(card, index) {
      ...
      if (this.currentCard_ == -1)
        ...
        this.selectCard(this.currentCard_ + 1, 'add', false, true, true);
    ...
    removeCardAtIndex: function(index) {
      ...
      if (this.cards_.length == 0)
        ...
        this.selectCard(this.currentCard_ - 1, 'remove', false, true);
     ...
     * @param {string=} switchType which usage to send to Conduit.
	 * @param {boolean=} opt_animate Whether to animate.
     */
    selectCardByValue: function(newCard, switchType, opt_animate) {
      var i = this.cards_.indexOf(newCard);
      assert(i != -1);
      this.selectCard(i, switchType, opt_animate);
    },
    ...
    transformToCurrentCard_: function(opt_animate) {
    	...
    	this.doTransform_(opt_animate);

    	return true;
    },

	doTransform_: function(opt_animate) {
		var transition = '';
		if (opt_animate) {
    ...
    onDragEnd_: function(e) {
      ...
      this.selectCard(newCardIndex, 'drag', true);
    },
