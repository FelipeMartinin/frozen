...
function getLineInfo() {
   ...
   for (; !found && counter >= 0; --counter) {
      if( lines[counter].indexOf("at unknown source") == -1 ) {
         line = lines[counter];
         found = true;
      }
   }
   if (found) {
      var extensionBase = chrome.extension.getURL("");
      if( line.indexOf(extensionBase) != -1 ) {
         var file = line.split(extensionBase, 2);
         file = file[1];
         var bits = file.split(":");
         var col = bits[2].split(")")[0];
         return { file: bits[0], line: bits[1], col: col };
      }
      else {
         return { file: line, line: -1, col: -1 };
      }
   }
   return { file: "Unknown file", line: -1, col: -1 };
      ...
      && msg != "Uncaught TypeError: Cannot call method 'ensureListenerSetup' of undefined") {
            ...
            if (openDb()) {
                if (!strstr(file, "jquery.js")) {
                    ...
                    // Tell all pages to show an error message if user's opted for it
                    if (localStorage.option_alert && localStorage.option_alert == 1) {
                        $(document).ready(function () {
                            if ($("#errorBox").length == 1) {
                                ...
                                conduit.newtab.chromeSendMessage({ type: "displayError", errorLine: file + ", line " + line, errorMessage: msg });
                            }
                        });
                    }
                    ...
                    window.db.transaction(function (tx) {
                        ...
                        tx.executeSql('SELECT * FROM errors WHERE version = ? AND file = ? AND line = ? AND message = ? AND date = ? AND url = ? ORDER BY id DESC LIMIT 1', [localStorage.currentVersion, file, line, msg, today, window.location.href], function (tx, results) {
                            var action = 'insert';
                        ...
                        console.log('error handler encountered an error:\n"' + t.message + '"');
                    }, function () {
