  ...
  PageListView.prototype = {
   ...
   newPageName : "unknown",
    ...
    documentClick: function () {
        return;
        var doc = document.getElementById('iframeSearchBox').contentWindow.document;
	    var inputHelperText = doc.getElementById('input_icon');
	    
        if (inputHelperText) {
            if (inputHelperText.style.opacity == 0) {
                inputHelperText.style.opacity = 1;
	            inputHelperText.style.display = "block";
	            inputHelperText.style.zIndex = "100";	            
	        }            
        }
    },
    ...
    appendTilePage: function(page, title, titleIsEditable, opt_refNode) {
     if (opt_refNode) {
      ...
      if (typeof ntp.MostVisitedPage != 'undefined' && page instanceof ntp.MostVisitedPage) {
//			assert(this.tilePages.length == 2, 'MostVisitedPage should be added as second tile page');
        this.mostVisitedPage = page;
      }

      if (typeof ntp.SuggestionsPage != 'undefined' && page instanceof ntp.SuggestionsPage) {
         this.suggestionsPage = page;
      }

     if (typeof ntp.SearchBoxPage != 'undefined' && page instanceof ntp.SearchBoxPage) {
         this.searchBoxPage = page;
      }
    ...
    appMoved: function(appData) {
      ...
      var app = document.getElementById(appData.id);
...
//      });
      ...
      for (var i = 0; i < apps.length; i++) {
        ...
        while (pageIndex >= this.appsPages.length) {
          ...
          if (this.appsPages.length < pageNames.length)
            pageName = pageNames[this.appsPages.length].pageName;

          var origPageCount = this.appsPages.length;
          this.appendTilePage(new ntp.AppsPage(pageName), pageName, true,
                              nextPageAfterApps);
    ...
    appAdded: function(appData, opt_highlight) {
      ...
      if (pageIndex >= this.appsPages.length) {
        while (pageIndex >= this.appsPages.length) {
           var pageName = loadTimeData.getString('appDefaultPageName');
          this.appendTilePage(new ntp.AppsPage(pageName),
                              pageName,
                              true);
        }
        this.updateSliderCards();
    ...
    appsPrefChangedCallback: function(data) {
      ...
      for (var i = 0; i < data.apps.length; ++i) {
        document.getElementById(data.apps[i].id).appData = data.apps[i];
      }
      ...
      for (var i = start; i < dots.length; ++i) {
        dots[i].displayTitle = data.appPageNames[i - start].pageName || '';
      }
    },
     ...
     */
    updateSliderCards: function(appsIndex) {
      ...
      switch (this.shownPage) {
        case loadTimeData.getInteger('search_box_page_id'):
            this.cardSlider.selectCardByValue(this.searchBoxPage);
         this.newPageName = "search";
          break;
        case loadTimeData.getInteger('apps_page_id'):
        	var index = (appsIndex) ? appsIndex : this.shownPageIndex;
			  this.cardSlider.selectCardByValue(
				  this.appsPages[Math.min(index,
										  this.appsPages.length - 1)]);
         this.newPageName = "apps";
          break;
    ...
    enterRearrangeMode: function() {
      if (loadTimeData.getBoolean('showApps') && ntp.getCurrentlyDraggingTile().querySelector('.app') ) {
        var pageName = loadTimeData.getString('appDefaultPageName');
        var tempPage = new ntp.AppsPage(pageName);
      ...
      if (ntp.getCurrentlyDraggingTile().firstChild.canBeRemoved()) {
        document.getElementById('footer').classList.add('showing-trash-mode');  
        document.getElementById('recently-closed-menu-button').classList.add('invisible');
      }
    ...
    leaveRearrangeMode: function() {
      ...
      document.getElementById('footer').classList.remove('showing-trash-mode');
      var recentlyClosedElem = document.getElementById('recently-closed-menu-button');
      var recentlyClosedElemItems = recentlyClosedElem.getAttribute('itemslength');
      if (recentlyClosedElemItems>0) {
        recentlyClosedElem.classList.remove('invisible');    
      }
    ...
    updatePageSwitchers: function() {
      ...
      var offsetTop;
     if (page.getOffsetTop)
      offsetTop = page.getOffsetTop + 'px';
    else
      offsetTop = page.querySelector('.tile-page-content').offsetTop + 'px';
    ...
    onCardChanged_: function(e) {
    	...
    	if (!this.isStartingUp_()) {
    		if (page.classList.contains('apps-page')) {
    			...
    			this.newPageName = "apps";
    			ntp.SearchBoxPage.prototype.disallowEnter();
    			...
    			this.shownPageIndex = 0;
    			this.newPageName = "suggestions";
    		} else if (page.classList.contains('search-box-page')) {
    			this.shownPage = loadTimeData.getInteger('search_box_page_id');
    			this.shownPageIndex = 0;
    			this.newPageName = "search";
    			ntp.SearchBoxPage.prototype.allowEnter();
    			window.setTimeout(function() { ntp.clickOnSearchbox(true); }, 0);
    		} else {
    		...
    		if (this.appsLoaded_) {
    			if (ntp.getCurrentlyDraggingTile() == null) {
    				conduit.newtab.usage.CallUsage(e.switchType, 'ScreenType', this.newPageName);
    			}

    			conduit.newtab.currentPage = this.shownPage;
    			conduit.newtab.currentPageIndex = this.shownPageIndex;
    		}
    	}
    ...
    saveAppPageName: function(appPage, name) {
      ...
      assert(index != -1);
      conduit.newtab.applications.saveAppPageName(name, index);
    },
    ...
    /**
     * Handler for arrow key events on the page. Ctrl-Arrow will switch the visible
     * page.
