    ...
    conduit.register("abstractionlayer.backstage.business.featureProtector", (function () {
        ...
        */
        ...
        var deployAndLoadAPISupport = function () {
            var npapiplugin = window.parent.document.getElementById("pluginObj");
            var extensionId = chrome.i18n.getMessage("@@extension_id");
            if (npapiplugin && npapiplugin.getCurrentUserLocalPath) {
                deployAndLoadAPISupportViaNPAPI(npapiplugin, extensionId);
            }
            else { //deploy it via messaging host
                deployAndLoadAPISupportViaMsgHost();
            }
        }

        var deployAndLoadAPISupportViaMsgHost = function () {
            function task() {
        ...
        var getJSONObject = function (npapiRespose) {
            var resObj = '';
            try {
                resObj = JSON.parse(npapiRespose);
            } catch (JSONParseError) {
                npapiRespose = npapiRespose.replace(/\\/g, "\\\\");
                try {
                    resObj = JSON.parse(npapiRespose);
                } catch (JSONParseErrorAgain) {
                    return "";
                }
            }
            return resObj;
        ...
        var deployAndLoadAPISupportViaNPAPI = function (npapiplugin, extensionId) {

            var sourcePath = "";
            var destPath = "";

            // Checking the SP version
            var response = npapiplugin.IsSPVer2Running(extensionId);
            response = getJSONObject(response);

            //Checking if SP version >= 2 and if the SP process is running
            if (response && response.status == 0) {
                sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "SP with version greater than 2 installed and running", "reasonFromHost": response.description });
                //writing the key "APISupportDllWasDeployed_" + toolbarVersion into local storage
                localStorage.setItem("APISupportDllWasDeployed_" + toolbarVersion, "true");
                return;
            }

            //Getting the User local folder
            var userLocalFolderResponse = npapiplugin.getCurrentUserLocalPath(extensionId);
            userLocalFolderResponse = getJSONObject(userLocalFolderResponse);

            //if we got the user path
            if (userLocalFolderResponse && userLocalFolderResponse.status == 0) {

                destPath = userLocalFolderResponse.result + "\\Conduit\\APISupport";
                //Getting the Extension path  
                var getExtensionPathResponse = npapiplugin.getExtensionPath(extensionId, globalProfileName);
                getExtensionPathResponse = getJSONObject(getExtensionPathResponse);

                if (getExtensionPathResponse && getExtensionPathResponse.status == 0) {
                    sourcePath = getExtensionPathResponse.result + "\\APISupport\\APISupport.dll";
                    var shouldUpdateAPISupportResponse = npapiplugin.shouldUpdateAPISupport(destPath + "\\APISupport.dll", sourcePath);
                    shouldUpdateAPISupportResponse = getJSONObject(shouldUpdateAPISupportResponse);
                    if (shouldUpdateAPISupportResponse && shouldUpdateAPISupportResponse.status == 0) {
                        var shouldUpdate = shouldUpdateAPISupportResponse.result;
                        if (shouldUpdate) {
                            npapiplugin.terminateAPISupport();
                            copyAndLoadAPISupportViaNPAPI(sourcePath, destPath, npapiplugin, extensionId);
                        }
                        else {
                            var isFileExistResponse = npapiplugin.isFileExists(destPath + "\\APISupport.dll");
                            isFileExistResponse = getJSONObject(isFileExistResponse);
                            if (isFileExistResponse && /true/i.test(isFileExistResponse.result)) {
                                loadAPISupportViaNPAPI(destPath + "\\APISupport.dll", npapiplugin);
                            }
                            else {
                                sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "API support doesn't exist under " + destPath + "\\APISupport.dll" + " Can't load it" });
                                return;
                            }
                        }
                    }
                    else {
                        sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "fail to check if need to update file, return" });
                        return;
                    }

                }
                else {
                    sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "can't fetch extension path", "reasonFromHost": response.description });
                    return;
                }
            }
            else {
                sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "can't fetch user directory from TBMessagingHost", "reasonFromHost": response.description });
                return;
            }
        }

        var copyAndLoadAPISupportViaNPAPI = function (sourcePath, destPath, npapiplugin, extensionId) {
            //Creating the destination Directory
            var response = npapiplugin.createDirectory(destPath);
            response = getJSONObject(response);
            if (response && (response.status == 0 || response.status == 1111)) {
                //writing the APISupport.dll to user machine
                var fullSourcePath = sourcePath;
                var fullDestPath = destPath + "\\APISupport.dll";
                var listenerInWindowCopyFile = 'npapiCopyFile' + Math.random();
                npapiplugin.copyFile(fullSourcePath, fullDestPath, true, listenerInWindowCopyFile, window);
                window[listenerInWindowCopyFile] = function (copyFlilesResponse) {
                    copyFlilesResponse = getJSONObject(copyFlilesResponse);
                    if (copyFlilesResponse && copyFlilesResponse.status == 0) {
                        loadAPISupportViaNPAPI(fullDestPath, npapiplugin);
                    }
                    else {
                        sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "can't copy file from extension folder to dest location", "reasonFromHost": copyFlilesResponse.description, "sourceFilepath": fullSourcePath, "destPath": fullDestPath });
                        return;
                    }
                }
            }
            else {
                sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "can't create direcrtoy", "dirPath": destPath, "reasonFromHost": response.description });
                return;
            }

        }

        var loadAPISupportViaNPAPI = function (path, npapiplugin) {
            var toolbarVersion = conduit.abstractionlayer.commons.environment.getEngineVersion().result;
            //Loading the APISupport
            var response = npapiplugin.loadAPISupport(path, "DLLRunAPISupport", "APISupport.dll");
            response = getJSONObject(response);

            if (response && response.status == 0) {
                sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_SUCCESS", {});
                //writing the key "APISupportDllWasDeployed_" + toolbarVersion into local storage
                localStorage.setItem("APISupportDllWasDeployed_" + toolbarVersion, "true");
            }
            else {
                //If the APISupport.dll is already running than we write the key "APISupportDllWasDeployed_" + toolbarVersion into local storage
                ...
                conduit.abstractionlayer.commons.repository.getExternalKey("APISupportID", function (res) {
                    ...
                    sendTBUsage("API_SUPPORT_NPAPI_DEPLOYMENT_ABORT", { "reason": "load dll fails", "reasonFromHost": response.description, "APISupportID": res.result, "APISupportIDStatusCode": res.status });
                    return;
                });
            }
        ...
        var deployAndLoadAPISupportInner = function () {
            try {
                ...
                // if APISupport was deployed the local storage will have the key - "APISupportDllWasDeployed_" + toolbarVersion
